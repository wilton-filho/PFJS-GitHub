...
function showPanelInfo($txt, $hasTimer, $countTimer, $srcImg, $fontSize) {
    ...
    if (!$hasTimer) {
        ...
        $($btnPanelInfo).click(function(){closePanelInfo($panelInfo)});
    }
    ...
    if ($hasTimer) showChronoPanel($panelInfo, $countTimer);
}
...
// Parâmetros:
//  - $id: id do painel de informações
function showChronoPanel($panelInfo, $countTimer) {
    $($panelInfo).children("div").children("p").text($countTimer);
    if ($countTimer > 0) setTimeout(showChronoPanel, 1000, $panelInfo, --$countTimer);
    else closePanelInfo($panelInfo);
}
